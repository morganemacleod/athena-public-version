...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  int  prob = pin->GetInteger("problem","prob");
  ...
  for (int i=is; i<=ie; i++) {
    ...
    // ADVECTION-WAVE PROBLEM
    if (prob==3) {
      Real vx = 0.1;
      phydro->u(IM1,k,j,i) =  den*vx * std::sin(pcoord->x2v(j))*std::cos(pcoord->x3v(k));
      phydro->u(IM2,k,j,i) =  den*vx * std::cos(pcoord->x2v(j))*std::cos(pcoord->x3v(k));
      phydro->u(IM3,k,j,i) = -den*vx * std::sin(pcoord->x3v(k));
      phydro->u(IEN,k,j,i) = 0.5*(SQR(phydro->u(IM1,k,j,i)) + SQR(phydro->u(IM2,k,j,i)) + SQR(phydro->u(IM3,k,j,i)) )/den;
    }
    ...
    if (NON_BAROTROPIC_EOS) {
      ...
      // WAVE PROBLEM
      if ((prob==2 | prob==3) & (rad < 5.0*rout)) {
	pres += 0.01*pa* std::exp(-rad*rad/(2.0*rout*rout));
      }
